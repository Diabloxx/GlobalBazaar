...
const SellerDashboard = () => {
  ...
  const { data: sellerOrders = [], isLoading: ordersLoading, error: ordersError } = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      try {
        console.log("Fetching seller orders with auth state:", { 
          isAuthenticated, 
          userRole: user?.role,
          userId: user?.id 
        });
        ...
        try {
          const json = JSON.parse(text);
          console.log("Successfully parsed orders response:", Array.isArray(json) ? `Array with ${json.length} items` : typeof json);
          return Array.isArray(json) ? json : [];
        } catch (parseError) {
